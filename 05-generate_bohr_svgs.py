...
def generate_orbital_box_svg(symbol, number, shells, group, electron_config='',
    ...
    """
    Vista 3 final — círculos con ↑↓ dentro, layout con wrap correcto.
    Fusión: círculos+Ne⁻ de Velvet + wrap/overflow fix propio.
    """
    if electron_config:
    ...
    lines.append("""<defs>
  <filter id="glow"><feGaussianBlur stdDeviation="1.8" result="b"/>
  <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
        ...
        for (l_char, spins, color, label, count) in items:
            ...
            for bi, spin in enumerate(spins):
                ...
                lines.append(f'<rect x="{bx:.1f}" y="{by:.1f}" '
                             ...
                             f'stroke-width="0.7" opacity="0.35" rx="3"/>')
                if spin == 'down_up':
                    for xi, arrow in [(-3.2, '↑'), (3.2, '↓')]:
                        lines.append(f'<circle cx="{bcx+xi:.1f}" cy="{bcy:.1f}" r="3.8" '
                                    f'fill="{color}" filter="url(#glow)" opacity="0.9"/>')
                        lines.append(f'<text x="{bcx+xi:.1f}" y="{bcy+2.5:.1f}" '
                                    f'text-anchor="middle" font-size="5.5" '
                                    f'font-family="sans-serif" fill="#060d1a" '
                                    f'font-weight="bold">{arrow}</text>')
                elif spin == 'up':
                    lines.append(f'<circle cx="{bcx:.1f}" cy="{bcy:.1f}" r="4.5" '
                                f'fill="{color}" filter="url(#glow)" opacity="0.9"/>')
                    lines.append(f'<text x="{bcx:.1f}" y="{bcy+2.5:.1f}" '
                                f'text-anchor="middle" font-size="6" '
                                f'font-family="sans-serif" fill="#060d1a" '
                                f'font-weight="bold">↑</text>')
                else:
                    lines.append(f'<circle cx="{bcx:.1f}" cy="{bcy:.1f}" r="4" '
                                f'fill="{color}" opacity="0.08"/>')
            ex = x_cur + degen * (box_w + box_gap) + 2
            lines.append(f'<text x="{ex:.1f}" y="{cy_row + 3:.1f}" '
                         f'font-family="monospace" font-size="6.5" '
                         f'fill="{color}" opacity="0.55">{count}e⁻</text>')
            x_cur += degen * (box_w + box_gap) + sub_gap + 18
...
def generate_bohr_svg(symbol, number, shells, group, electron_config='', W=320, H=320,
                      cpk_color=None, el_color=None):
    """
    Genera el SVG del diagrama de Bohr con subshells distinguibles.
    """
    cx, cy = W // 2, H // 2
    ...
    total_rings = sum(len(subs) for subs in subshell_data.values())

    nucleus_r   = max(10, min(18, 20 - n_shells))
    max_orbit_r = (min(W, H) // 2) - 22  # margen fijo de 22px
    electron_r  = max(2.0, min(4.5, 5.5 - n_shells // 2))

    # Distribuir radios proporcional al total de anillos
    # gap_shell > gap_sub para mantener agrupación visual por capa
    available     = max_orbit_r - nucleus_r
    # Peso total: cada capa aporta 3 unidades + cada subshell 1 unidad
    total_weight  = n_shells * 3 + total_rings
    unit          = available / max(total_weight, 1)
    gap_shell     = unit * 3   # salto entre capas
    gap_sub       = unit * 1   # salto entre subshells dentro de capa

    # Color del núcleo — CPK del JSON si disponible, si no por grupo
    if cpk_color:
        nuc_color = f'#{int(str(cpk_color).replace("0x",""), 16):06x}'
    elif el_color:
        nuc_color = f'#{int(str(el_color).replace("0x",""), 16):06x}'
    ...
    lines.append(f'<svg xmlns="http://www.w3.org/2000/svg" '
                 ...
                 f'style="background:#060d1a" '
                 f'data-symbol="{symbol}" data-shells="{",".join(str(s) for s in shells)}">')
  ...
  <filter id="nglow" x="-100%" y="-100%" width="300%" height="300%">
    ...
    # ── Anillos por capa y subshell ───────────────────────────────────────────
    shell_idx = 0  # índice global de shell para el id SVG
    current_r = nucleus_r

    for n, subshells in subshell_data.items():
        current_r += gap_shell  # salto entre capas

        for si, (l_char, count) in enumerate(subshells):
            if si > 0:
                current_r += gap_sub  # pequeño offset entre subshells de la misma capa

            r     = current_r
            color = subshell_color(l_char, n)
            label = f'{n}{l_char}'

            # Grupo con id para resaltar desde JS (shell-0, shell-1, ...)
            lines.append(f'<g id="shell-{shell_idx}" class="bohr-shell" '
            ...
            lines.append(
                ...
                f'fill="none" stroke="{color}" stroke-width="0.6" opacity="0.25" '
                f'class="orbit-ring"/>'
            )
            ...
            for j in range(count):
                ...
                lines.append(
                    f'  <circle cx="{ex:.2f}" cy="{ey:.2f}" r="{electron_r:.1f}" '
                    f'fill="{color}" filter="url(#glow)" opacity="0.85" '
                    f'class="electron"/>'
                )
    ...
    # ── Núcleo ────────────────────────────────────────────────────────────────
    lines.append(f'<g id="nucleus" class="bohr-nucleus">')
    lines.append(
        f'  <circle cx="{cx}" cy="{cy}" r="{nucleus_r}" '
        f'fill="{nuc_color}" filter="url(#nglow)" opacity="0.9"/>'
    )
    lines.append(
        f'  <circle cx="{cx}" cy="{cy}" r="{nucleus_r * 0.5:.1f}" '
    ...
    lines.append(
        ...
        f'font-size="{font_size}" font-weight="bold" fill="#fff" opacity="0.95">'
        f'{symbol}</text>'
    )
    lines.append(f'</g>')
    ...
    # ── Número atómico ────────────────────────────────────────────────────────
    lines.append(
        f'<text x="{cx}" y="{cy - nucleus_r - 5:.1f}" '
        f'text-anchor="middle" font-family="monospace" '
        f'font-size="8" fill="#334" opacity="0.6">{number}</text>'
    )
    ...
    lines.append(
        f'<text x="{cx}" y="{H - 6}" '
        f'text-anchor="middle" font-family="monospace" '
        f'font-size="7" fill="#334" opacity="0.45">{shell_str}</text>'
    )
